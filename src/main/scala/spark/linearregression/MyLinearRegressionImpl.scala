...
object Helper {
  def rmse(labelsAndPreds: RDD[(Double, Double)]): Double = {
    return Math.sqrt(labelsAndPreds.map(e => Math.pow(e._1 - e._2,2)).reduce(_+_) / labelsAndPreds.count)
  }

  def predictOne(weights: Vector, features: Vector): Double = {
    return VectorHelper.dot(weights,features)
  }

  def predict(weights: Vector, data: RDD[Instance]): RDD[(Double, Double)] = {
    return data.map(row => (row.label, predictOne(weights, row.features)) )
  }
}
...
class MyLinearRegressionImpl(override val uid: String)
  ...
  def gradientSummand(weights: Vector, lp: Instance): Vector = {
    return VectorHelper.dot(lp.features,Helper.predictOne(weights, lp.features) - lp.label)
  }

  def gradient(d: RDD[Instance], weights: Vector): Vector = {
    return d.map(gradientSummand(weights,_)).reduce(VectorHelper.sum)
  }
